...

{
	...
	private String pin, firstName, youFirstName;
	...
	// and PIN ( ڰ Է ¹ ȣ PIN)
	private String myAccountNumber, yourAccountNumber, userPIN;

	// instance variable used to store account balance
	private double balance, youBalance;

	private Connection myConnection;
	private Statement myStatement;
	private ResultSet myResultSet;
	
	private ResultSet youResultSet;
	...
	private void createUserInterface()
	{
		...
		// JFrame GUI Ʈ ̱ ؼ content pane ; Ѵ .
		...
		messageJTextArea.setBounds( 40, 16, 288, 88 ); // ( 𼭸 x, y, , )
		...
		messageJTextArea.setEditable( false ); // Ұ ϰ
		...
		oneJButton.setBorder(BorderFactory.createRaisedBevelBorder() );
		...
		enterJButton.setBounds( 149, 9, 72, 24 );
		...
		// set up transfer JButton
				transferJButton = new JButton();
				transferJButton.setBounds( 149, 90, 72, 24 );
				transferJButton.setText( "Transfer" );
				transferJButton.setBorder(BorderFactory.createRaisedBevelBorder() );
				...
				transferJButton.addActionListener(
						new ActionListener() // anonymous inner class
						{
							...
							public void actionPerformed( ActionEvent event )
							{
								transferJButtonActionPerformed( event );
							}
						} // end anonymous inner class
						); // end call to addActionListener
		...
		doneJButton.setBounds( 149, 117, 72, 24 );
	...
	private void zeroToNineJButtonActionPerformed( String number )
	{
		...
		if ( action == ENTER_PIN )
		{
			...
			numberJTextField.setText(numberJTextField.getText() + "*" ); // ǥ ο
		}
	...
	private void enterJButtonActionPerformed( ActionEvent event )
	{
		if ( action == ENTER_PIN ) // checking PIN
		{
			...
			if ( userPIN.equals( pin ) )
			{
				...
				// enable balanceJButton, withdrawJButton and transferJButton 
		...
		else if ( action == WITHDRAWAL ) // process withdrawal
		{
			...
			// process withdrawal
			withdraw( 
					Double.parseDouble( numberJTextField.getText() ) ); // ڿ Ǽ ȯ Ѵ .
			...
			int money = Integer.parseInt(numberJTextField.getText());
			if(money<=balance) {
				balance = balance - money;
				youBalance = youBalance + money;
				
				//데이터베이스에 업데이트
				updateBalance();
				
				//성공했다는 메시지와 함께 본인 계좌, 상대 계좌의 소유주 이름, 잔액 보여주기
				messageJTextArea.setText(firstName+" 계좌에서 "+youFirstName+" 계좌로 "+"\n"+money+"가 이체되었습니다.\n"
						+firstName+"의 잔액은 "+balance+", \n"+youFirstName+"의 잔액은 "+youBalance+"입니다.");
				disableKeyPad();
				numberJTextField.setText("");
			}
			else {
				messageJTextArea.setText("잔액이 부족합니다. 이체할 금액을 다시 입력해주세요.");
				enableKeyPad();
				numberJTextField.setText("");
			}
		}
	...
	private void transferJButtonActionPerformed(ActionEvent event) {
		action = TRANSFER;
		...
		messageJTextArea.setText("이체 대상 계좌번호를 선택하세요.");
		accountNumberJComboBox.setSelectedIndex( 0 );
	}
	...
	private void accountNumberJComboBoxItemStateChanged(ItemEvent event )
	{
		...
		// in process
		if ( ( event.getStateChange() == ItemEvent.SELECTED ) && // Ͼ ,
				( accountNumberJComboBox.getSelectedIndex() != 0 )&& action!=TRANSFER ) // ΰ ߴٸ ...
		{
		...
		else if((event.getStateChange() == ItemEvent.SELECTED)&& ( accountNumberJComboBox.getSelectedIndex() != 0 ) && action==TRANSFER) {
			yourAccountNumber = 
					( String ) accountNumberJComboBox.getSelectedItem();
			try {
				youResultSet = myStatement.executeQuery("SELECT firstName, balanceAmount FROM accountInformation "+
			       "WHERE accountNumber = '"+yourAccountNumber+"'");

				//get next result
				if(youResultSet.next()) {
					youFirstName = youResultSet.getString("firstName");
					youBalance = youResultSet.getDouble("balanceAmount");
				}
				youResultSet.close(); // close myResultSet
			} // end try
			catch(SQLException exception) {
				exception.printStackTrace();
			}
			messageJTextArea.setText(youFirstName+"님의 계좌로 보낼 금액을 입력하세요.");
			enableKeyPad();
		}
	...
	private void updateBalance()
	{
		try {
			...
			if(action == TRANSFER) {
				myStatement.executeUpdate("UPDATE accountInformation"+
					       " SET balanceAmount = "+youBalance+" WHERE " +
